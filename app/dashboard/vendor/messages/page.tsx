                            ...
                            <div className="flex-1 overflow-y-auto">
                                {threads.map((thread) => (
                                    <button key={thread.id} onClick={() => selectThread(thread)}
                                        ...
                                        <p className="text-xs text-gray-400 mt-0.5">{thread.messages[0]?.content?.slice(0, 50) || "No messages"}</p>
                                    </button>
                                ))}
                            </div>
                        )}
                        ...
                        {selectedThread ? (
                            <>
                                <div className="p-4 border-b border-gray-100 flex items-center gap-3">
                                    <button onClick={() => setSelectedThread(null)} className="lg:hidden p-1 text-gray-400 cursor-pointer"><ArrowLeft className="h-4 w-4" /></button>
                                ...
                                <div className="flex-1 overflow-y-auto p-4 space-y-3">
                                    {messages.map((msg) => (
                                        <div key={msg.id} className={`flex ${msg.senderId === user?.id ? "justify-end" : "justify-start"}`}>
                                            <div className={`max-w-[70%] px-4 py-2.5 rounded-2xl text-sm ${msg.senderId === user?.id ? "bg-primary-600 text-white rounded-br-md" : "bg-gray-100 text-gray-900 rounded-bl-md"
                                                }`}>
                                                <p>{msg.content}</p>
                                                <p className={`text-[10px] mt-1 ${msg.senderId === user?.id ? "text-primary-200" : "text-gray-400"}`}>{formatDateTime(msg.createdAt)}</p>
                                            </div>
                                        </div>
                                    ))}
